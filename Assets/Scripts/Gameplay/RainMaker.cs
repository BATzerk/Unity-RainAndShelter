...
public class RainMaker : MonoBehaviour {
    ...
    // Properties
    [SerializeField] private float RainVolume = 0.8f; // from 0 to 1!
    ...
    private void FixedUpdate() {
        ...
        {
            // Update target
            float targetDry  = Mathf.Lerp(0f, 0.2f, RainVolume);
            float targetRain = Mathf.Lerp(0.3f, 0.7f, RainVolume);
            screenFogAlphaTarget = player.IsInRain ? targetRain : targetDry;
            // Apply target
